...
uses
  ...
  AudioFileBasics, //Apev2Tags, ApeTagItem, FlacFiles, M4AFiles,
  ID3v2Frames, Mp3Files, OggVorbisFiles, WavFiles;
...
begin
    ...
    if (ext = '.mp3') or (ext = '.mp2') or (ext = '.mp1') then
    begin
        fBaseAudioFile := TMP3File.Create;
        fFileType := at_Mp3;
    end
    else
    if (ext = '.ogg') or (ext = '.oga') then
    begin
        fBaseAudioFile := TOggVorbisFile.Create;
        fFileType := at_Ogg;
    end
    else
{    if (ext = '.flac') or (ext = '.fla') then
    begin
    ...
    if (ext = '.mpc') or (ext = '.mp+') or (ext = '.mpp') then
    begin
        fBaseAudioFile := TMusePackFile.Create;
        fFileType := at_MusePack;
    end
    else
    if (ext = '.ofr') or (ext = '.ofs') then
    begin
