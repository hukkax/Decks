...
{$IFDEF MACOS}
  {$IFDEF IOS}
    bassencdll = 'bassenc.framework/bassenc';
  {$ELSE}
...
function BASS_Encode_StopEx(handle:DWORD; queue:BOOL): BOOL; {$IFDEF MSWINDOWS}stdcall{$ELSE}cdecl{$ENDIF}; external bassencdll;
...
function BASS_Encode_UserOutput(handle:HENCODE; offset:QWORD; buffer:Pointer; length:DWORD): BOOL; {$IFDEF MSWINDOWS}stdcall{$ELSE}cdecl{$ENDIF}; external bassencdll;
...
function BASS_Encode_CastInit(handle:HENCODE; server,pass,content,name,url,genre,desc,headers:PAnsiChar; bitrate,flags:DWORD): BOOL; {$IFDEF MSWINDOWS}stdcall{$ELSE}cdecl{$ENDIF}; external bassencdll;
